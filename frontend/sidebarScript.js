                ...
                if(element["experiment"]===exp){
                    ...
                    //prevent duplicate cell lines to be populated each a time
                        ...
                        if(!cellLineListHTML.includes(element["cell_line"])){
                            if(element["cell_line"].includes(" ")){
                                var forId = element["cell_line"].replace(/ /g, '___');
                                cellLineListHTML += "<p class = 'cellLineP' id = '" + forId+ "' >" + element["cell_line"] + "   +</p>" + "<div id = '"+ forId + "' class = 'cellLine " + exp+ "'></div>";
                            }else{
        ...
        function populateDamageType(cellLine, parentExp, bool){
            console.log("cell " + cellLine)
            //create placeholder for cell lines w/ spaces in them, only if it works.
            var forId;
            try {
                forId = cellLine.replace(/ /g, '___');
            } catch (error) {
                
            }
            
            var damageHTML = "<h5 class = 'tab3' id = 'damageList"+ cellLine+"'><b>Damage Type:</b></h5>";
            ...
            experimentList.forEach(element =>{
                ...
                if(element["cell_line"]===cellLine && element["experiment"]===parentExp){
                    ...
                    if(!damageHTML.includes(element["damage"])){
                        ...
                        damageHTML += "<div id = '"+element["damage"]+"' class = 'tab3 damage "+forId+ " " + parentExp + "'></div>";
                    }
                }
            })
            ...
            experimentList.forEach(element =>{
                if(element["damage"] === damage && element["cell_line"] === parCell && element["experiment"]===grandparentExp){
                    if(!tracksHTML.includes(element["title"])){
                        tracksHTML += "<p class = 'tab4 track "+ damage+" "+ grandparentExp+ "' id = '"+element["title"]+"'>"+ element["title"] + "</p>";
                    }
                }
            })
            if($("div#"+damage + "."+parCell).text().indexOf("Tracks") === -1){
                
                $("div#"+damage + "."+parCell).html(tracksHTML)
            }
            return;
        }
        ...
        $(document).on("click", ".cellLineP", function(){
            ...
            if(!selected){
                ...
                var id = $(this).attr("id").replace(/___/g, ' ')
                
                
                console.log("id in event " + id)
                
                return(populateDamageType(id, parentExp, true))
        ...
        $(document).on("click", ".damageP", function(){
            ...
            if(!selected){
                ...
                var newHTML = oldHTML.split("   +")[0] + "   -";
                $(this).html(newHTML);
                console.log("parcellline " + parCellLine   + "parexp " + parExp)
        ...
        $(document).on("click", ".track", function(){
            ...
            if(!selected){
                $(this).addClass("highlight")
                $("#selectedTracks").append("," + $(this).attr("id"));
                
                var newText = $("#selectedTracks").val().replace($(this).attr("id"), "");
                $("#deselectedTracks").text(newText)
                console.log("before logi " + newText)
                
                //$("#deselectedTracks").val(newText);
                return(console.log("adding track"+ $(this).attr("id")+ "to browser"))
            }else{
                $(this).removeClass("highlight")
                $("#deselectedTracks").append(","+$(this).attr("id"));
                var newText = $("#selectedTracks").val().replace($(this).attr("id"), "");
                $("#selectedTracks").val(newText);
